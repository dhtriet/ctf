...
01
Đọc trang, tìm sink thật
Trang là một scene Three.js dựng cảnh “sinh viên ngồi một mình trong phòng tối” — toàn bộ phần nhìn đều là nhiễu. Tài sản được nạp chỉ có hai file, đều là engine render:
...
Không có logic nghiệp vụ nào trong đó ⇒ đọc script inline trong index.html. Hàm submit() lộ toàn bộ attack surface:

index.html — hàm submit()
...
Một endpoint, một param expr, response JSON với ba khoá answer / message / status. Ô nhập bị cắt ở 120 ký tự phía client — curl không dính giới hạn đó.
...
Body lỗi có đúng bộ khoá timestamp / status / error / path — đây là JSON lỗi mặc định của BasicErrorController trong Spring Boot. Java/Spring ⇒ nghĩ ngay tới SpEL, OGNL, MVEL.
...
EL####E cùng câu “Property or field … cannot be found on null” là mã lỗi riêng của SpEL. Server đang chạy đúng dạng:

new SpelExpressionParser().parseExpression(expr).getValue();
04
Đo không gian cho phép của EvaluationContext
SpEL có hai chế độ, và chúng quyết định bài còn đường hay không: SimpleEvaluationContext chặn T(), new, gán biến; StandardEvaluationContext thì mở hết. Ba phép thử là đủ phân biệt:
...
Cả ba đều qua ⇒ StandardEvaluationContext. RCE gần như chắc chắn; chướng ngại duy nhất còn lại là blacklist.

05
Lập bản đồ blacklist bằng string literal
Thay vì bắn thẳng một payload RCE rồi đoán mò vì sao nó hỏng, hãy thử từng từ khoá riêng lẻ 
...
Đoán từ hành vi là đủ để đi tiếp, nhưng đọc hằng số trong class thì chắc chắn hơn hẳn:
...
Đúng hai substring bị cấm, hết. JAR chỉ chứa hai class của app: NeuroController và PublicStaticVoidApplication.
...
Còn một chi tiết nữa: app gọi String.valueOf(obj) trên kết quả, nên payload buộc phải trả về một String. start() trả Process. Chuỗi biến đổi cần thiết:
...
exec java -Xmx48m -Xms32m -XX:+UseSerialGC -XX:MaxMetaspaceSize=48m \
     -jar /app/app.jar`
Flag không nằm cứng trong app, nó là dynamic flag của GZCTF, inject qua env rồi mới ghi ra file. Kiểm chéo ba nguồn độc lập:
